...
def install(workspace_app: typer.Typer, analysis_app: typer.Typer) -> None:
    @workspace_app.command("capabilities")
    def capabilities(
        json_output: Annotated[bool, typer.Option("--json")] = False,
    ) -> None:
        _echo(capability_matrix(), json_output)

    @workspace_app.command("runs")
    def runs(
    ...
    @workspace_app.command("aggregate")
    def aggregate(
    ...
    def migrate(
        ...
        def action() -> dict[str, object]:
            ...
            return {
                ...
                "written": write and report.changed,
            }

        _run(action, json_output=json_output)

    @analysis_app.command("context-create")
    def context_create(
    ...
    @analysis_app.command("contexts")
    def contexts(
        workspace: Annotated[Path, typer.Option("--workspace")] = Path("."),
        json_output: Annotated[bool, typer.Option("--json")] = False,
    ) -> None:
        _run(
            lambda: {"contexts": NotebookContextStore(workspace).list()},
            json_output=json_output,
        )

    @analysis_app.command("assistant-plan")
    def assistant_plan(
        goal: str,
        run_id: Annotated[list[str] | None, typer.Option("--run")] = None,
        artifact_id: Annotated[list[str] | None, typer.Option("--artifact")] = None,
        ...
        _echo(
            AssistantEngine(str(workspace.resolve()))
            .plan(request)
            .model_dump(mode="json"),
            json_output,
        )

    @analysis_app.command("assistant-apply")
    def assistant_apply(
        plan_path: Path,
