...
def _register(app, server_module) -> None:
    ...
    script_path = Path(__file__).with_name("ui") / "static" / "pipeline-extension.js"
    index_path = Path(server_module.__file__).with_name("static") / "index.html"

    @app.middleware("http")
    async def pipeline_root(request: Request, call_next):
    ...
    @app.get("/api/extensions/pipeline.js")
    def pipeline_javascript():
        return Response(
